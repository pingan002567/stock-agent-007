...
CAPABILITIES: dict[str, CapabilitySpec] = {
    ...
    "northbound_hold": CapabilitySpec(
        name="northbound_hold",
        description="沪深股通持股快照（北向持股数量/占比）",
        providers=("tushare",),
        params=(CapabilityParam("symbol", "str", True, "A 股代码"),),
        returns_summary="as_of, vol, ratio, exchange；含日度披露变更说明",
        limitations="仅 Tushare；2024-08-20 起交易所停发日度北向资金，改为季度披露，as_of 可能滞后",
        rate_limit_hint="Tushare 按积分限流",
    ),
    "margin_detail": CapabilitySpec(
        name="margin_detail",
        description="个股融资融券明细最新值",
        providers=("tushare",),
        params=(CapabilityParam("symbol", "str", True, "A 股代码"),),
        returns_summary="rzye/rqye/rzmre/rzrqye 等",
        limitations="仅 Tushare；约 2000 积分起",
        rate_limit_hint="Tushare 按积分限流",
    ),
    "lhb": CapabilitySpec(
        name="lhb",
        description="龙虎榜近期上榜摘要（含机构席位名截断）",
        providers=("tushare",),
        params=(CapabilityParam("symbol", "str", True, "A 股代码"),),
        returns_summary="on_list, items[{trade_date,reason,net_amount,…}]",
        limitations="仅 Tushare；近期末上榜时 on_list=false 不算失败",
        rate_limit_hint="回溯交易日会多次调用，勿高频",
    ),
    "share_float": CapabilitySpec(
        name="share_float",
        description="限售股解禁（近期已解禁 + 未来窗口）",
        providers=("tushare",),
        params=(CapabilityParam("symbol", "str", True, "A 股代码"),),
        returns_summary="upcoming[] / recent[]",
        limitations="仅 Tushare；窗口约过去 90 天～未来 180 天",
        rate_limit_hint="Tushare 按积分限流",
    ),
}
