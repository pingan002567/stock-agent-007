...
export function Rail() {
  const { currentScreen, setCurrentScreen, globalLoading, appDataCache, lastRefreshTime } = useAppState();
    ...
    <aside className={`rail${globalLoading ? " rail-loading" : ""}`}>
      ...
      <nav className="rail-nav">
        {navItems.map((item) => (
          <button
            ...
            onClick={() => { if (!globalLoading) setCurrentScreen(item.screen); }}
            title={item.label}
            disabled={globalLoading}
          >
            {icons[item.screen]}
            <span>{item.label}</span>
            {item.screen === "overview" && inboxHigh > 0 && (
              <span className="rail-badge">{inboxHigh > 99 ? "99+" : inboxHigh}</span>
            )}
          </button>
        ))}
      </nav>
      <div className="rail-footer">
        <button
          ...
          onClick={() => { if (!globalLoading) setCurrentScreen("settings"); }}
          title="设置"
          disabled={globalLoading}
        >
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
